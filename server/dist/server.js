    ...
    function analyze(text, path, ws2, config2 = exports2.DEFAULT_CONFIG) {
      ...
      (0, tree_1.walk)(root, (x) => {
        ...
        if (id && !types.defs.has(id.text)) {
          add(id, `unknown type "${id.text}"; declare it with TYPE or import it with USE`, config2.unknownComponentSeverity);
        } else if (id) {
          mark(id, "type");
        }
      ...
      function checkScope(scope) {
        ...
        const checkFieldAccess = (fa) => {
          ...
          if (hearBlockFor(fa, name2)) {
            ...
            const v = values.get(name2);
            if (!v) {
              add(obj, `"${name2}" is not declared here; fields like ${name2}.freq are read from a HEAR's vibration, inside HEAR(${name2} IN ...)`, config2.undeclaredSignalSeverity);
              return;
            }
            ty = v.type;
          }
...
var require_features = __commonJS2({
  "build/features.js"(exports2) {
    ...
    function portLine(p, arrow) {
      const pins = pinsOf(p);
      const name2 = pins ? `${pins}[${p.name}]` : p.name;
      return `${arrow} ${name2}: ${p.type ?? "?"}${p.optional ? " = 0" : ""}`;
    }
    ...
    function targetAt(model, ws2, docPath, line, character) {
      ...
      if (n.type === "field_name" && parent) {
        if (parent.type === "field_access") {
          return n.text in analyze_12.FIELDS.vibration ? { kind: "vib_field", name: n.text } : void 0;
        }
        if (parent.type === "type") {
    ...
    function hoverFor(t, model) {
      ...
      switch (t.kind) {
        ...
        case "vib_field":
          return [code(`vib.${t.name}`), `A vibration's ${t.name}: ${analyze_12.FIELDS.vibration[t.name]}`].join("\n\n");
        case "module":
...
${t.path}`;
    ...
    function hearVarsAbove(lines, line) {
      ...
      for (let i2 = line - 1; i2 >= 0; i2--) {
        const m = /^\s*HEAR\s*\(\s*([A-Za-z_&][\w&]*)\s+IN\b/.exec(lines[i2]);
    ...
    function completionsAt(model, ws2, docPath, text, line, character, onlySpecific = false) {
      ...
      if (m) {
        if (hearVarsAbove(lines, line).has(m[1])) {
          return Object.entries(analyze_12.FIELDS.vibration).map(([k, v]) => item(k, vscode_languageserver_1.CompletionItemKind.Field, v));
        }
        return [];
      }
